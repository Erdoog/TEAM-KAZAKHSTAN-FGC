...
//    var clawController: ClawController? = null;
    ...
    var leftFrontDrive:  Motor? = null;
    var rightFrontDrive: Motor? = null;
    var leftRearDrive:   Motor? = null;
    var rightRearDrive:  Motor? = null;
    ...
    override fun init() {
        ...
        leftFrontDrive = Motor(hardwareMap, "leftFront")
        rightFrontDrive = Motor(hardwareMap, "rightFront")
        leftRearDrive = Motor(hardwareMap, "leftBack")
        rightRearDrive = Motor(hardwareMap, "rightBack")
...
//        leftRearDrive!!.setZeroPowerBehavior(Motor.ZeroPowerBehavior.BRAKE);
//        rightRearDrive!!.setZeroPowerBehavior(Motor.ZeroPowerBehavior.BRAKE);
...
//        rightFrontDrive!!.setZeroPowerBehavior(Motor.ZeroPowerBehavior.BRAKE);
        ...
        leftFrontDrive!!.ACHIEVABLE_MAX_TICKS_PER_SECOND = 3000.0;
        rightFrontDrive!!.ACHIEVABLE_MAX_TICKS_PER_SECOND = 3000.0;
        leftRearDrive!!.ACHIEVABLE_MAX_TICKS_PER_SECOND = 3000.0;
        rightRearDrive!!.ACHIEVABLE_MAX_TICKS_PER_SECOND = 3000.0;
...
//            liftsController!!.target = 0.0;
        ...
        if (driver1!!.getButton(GamepadKeys.Button.X))
            liftsController!!.target = 12600.0;
//        if (driver1!!.getButton(GamepadKeys.Button.B))
//            liftsController!!.target = 1600.0;
        if (driver1!!.getButton(GamepadKeys.Button.A))
            liftsController!!.target = 0.0;

        if (liftsController!!.target > 15200.0)
            liftsController!!.target = 15200.0;

        if (driver1!!.getButton(GamepadKeys.Button.DPAD_UP))
            liftsController!!.forceMove(0.6);
        if (driver1!!.getButton(GamepadKeys.Button.DPAD_DOWN))
            liftsController!!.forceMove(-0.6);

        liftsController!!.update();

        var bottomIntakeInput = 0.0;
...
//            bottomIntakeInput -= 1;
//        intakeController!!.setBottomVelocity(bottomIntakeInput);
...
//        clawController!!.update();
    ...
    fun arcadeDrive(forwardInput: Double, turnInput: Double, elapsedTime: Double){
        ...
        telemetry.addData("leftFront", leftFrontDrive!!.correctedVelocity);
        telemetry.addData("rightFront", rightFrontDrive!!.correctedVelocity);
        telemetry.addData("leftRear", leftRearDrive!!.correctedVelocity);
        telemetry.addData("rightRear", rightRearDrive!!.correctedVelocity);
    }
}